...
> Поле `sensor` у [variants.json](../../fixtures/variants.json) для більшості варіантів — `BME280`: це тип завдання (температура по I²C). **У Wokwi завжди працюєте з BMP180** з папки вище.
>
> **Варіанти OLED (4, 6, 9):** замість датчика — дисплей **SSD1306** (I²C **0x3C**). Файли `main.py` + `diagram.json` з [lab04-i2c-oled/](../../wokwi/lab04-i2c-oled/) + драйвер `ssd1306.py` з [wokwi/lib/](../../wokwi/lib/ssd1306.py). Завдання те саме по суті (I²C-обмін + Logic Analyzer), але виводите **прізвище латиницею** на екран; у `main.py` замініть `SURNAME`. `i2c.scan()` очікувано покаже `0x3c`.
